...
def generate_sql_from_dataframe(
    ...
    *,
    ...
    """
    Runs Qwen over the entire dataframe:
      - generates SQL for original & paraphrased questions
      - evaluates correctness
      - writes Qwen* columns into the same df
      - optionally saves CSV (qwen_results.csv) and checkpoints every N rows
    """
    ...
    cols = ["qwen_para_correct", "qwen_original_correct"]
    if store_sql:
        cols += ["qwen_query_from_para", "qwen_query_from_original"]
    for c in cols:
    ...
    for i, row in paraphrased_df.iterrows():
        ...
        try:
            ...
            paraphrased_df.at[i, "qwen_para_correct"] = bool(para_result)
            paraphrased_df.at[i, "qwen_original_correct"] = bool(original_result)

            if store_sql:
                paraphrased_df.at[i, "qwen_query_from_para"] = query_para
                paraphrased_df.at[i, "qwen_query_from_original"] = query_original

            if (i + 1) % 50 == 0:
                logger.info(f"[Qwen] processed {i+1} rows...")

            if checkpoint_every and (i + 1) % checkpoint_every == 0 and result_path:
                ckpt_file = result_path / "qwen_results_intermediate.csv"
                ...
                logger.info(f"[Qwen] checkpoint saved at row {i+1}: {ckpt_file}")

        except Exception as e:
            logger.error(f"[Row {i}] qwen NL2SQL error: {e}")
            paraphrased_df.at[i, "qwen_para_correct"] = False
            paraphrased_df.at[i, "qwen_original_correct"] = False
            if store_sql:
                paraphrased_df.at[i, "qwen_query_from_para"] = None
                paraphrased_df.at[i, "qwen_query_from_original"] = None
    ...
    sql_cols = ["qwen_query_from_para", "qwen_query_from_original"] if store_sql else []
    flag_cols = ["qwen_para_correct", "qwen_original_correct"]
    ...
    if result_path:
        out = result_path / "qwen_results.csv"
        # Robust CSV settings prevent “broken” rows in Excel/simple viewers
        final_df.to_csv(
            out, index=False,
            quoting=csv.QUOTE_ALL, escapechar='\\', lineterminator='\r\n'
        )
        logger.info(f"Qwen evaluation complete. Results saved to: {out}")
